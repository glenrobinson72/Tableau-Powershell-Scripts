...
#
#   Thanks to Gavin Butler on work done with Chunking Payloads
#
###############################################
...
$global:api_ver = '3.8'
$script:chunkSize = 2097152	   ## 2MB or 2048KB
...
function TS-SignIn
{
 ...
 $global:username = $username
   ...
   if ($ImpersonateID -ne '') {$ImpersonateDetails = '<user id="' + $ImpersonateID + '" />'}

 if ($Token -ne '')
   {
       # generate body for PAT sign in
     $signin_body = ('<tsRequest>
	<credentials personalAccessTokenName="' + $TokenName + '" personalAccessTokenSecret="' + $Token + '" >
   <site contentUrl="’ + $siteID +’"/>
    ...
    {
   # generate body for User / Password sign in
    $signin_body = (’<tsRequest>
  <credentials name=“’ + $username + ’” password=“’+ $password + ’” >
   <site contentUrl="’ + $siteID +’"/>' + $ImpersonateDetails + '
  </credentials>
 </tsRequest>’)
 ...
 try
  {
   ...
   "Site Name: " + $siteID + " " + $global:siteID
   #TS-ServerInfo
    if ($ImpersonateUser -ne '') 
    
      {
        $response = Invoke-RestMethod -Uri ${protocol}://$server/api/$api_ver/sites/$global:siteID/users?filter=name:eq:$ImpersonateUser -Headers $headers -Method Get
        $ImpersonateID = $response.tsResponse.Users.User.ID
        $global:Resp = $response.tsresponse
        $ImpersonateID

        If ($ImpersonateID -ne '')
         {
          "Impersonating " + $ImpersonateUser
          TS-SignIn -server $server -username $username -password $password -protocol $protocol  -siteID $SiteID -ImpersonateID $ImpersonateID
          }
          else
          { 
            "Unable to Impersonate: " +  $ImpersonateUser
          }
      }
  }

 catch {"Unable to Sign-In to Tableau Server: " + ${protocol}+"://"+$server + " :- " + $_.Exception.Message}
}
...
function TS-UpdateDataSource
{
 ...
 try
 {
   ...
   if ($NewProjectID -ne ''){ $NewProjectID} else {$NewProjectID = TS-GetProjectDetails -ProjectName $NewProjectName}

   $body = ""
   if ($NewProjectID -ne '') {$body += '<project id ="'+ $NewProjectID +'" />'}
...
function TS-UpdateDataSourceConnection
{
 ...
 try
 {
   ...
   $body
   $response = Invoke-RestMethod -Uri ${protocol}://$server/api/$api_ver/sites/$siteID/datasources/$DataSourceID/connections/$ConnectionID -Headers $headers -Method Put -Body $body
...
param(
  ...
  [validateset('Allow', 'Deny', 'Blank')][string[]] $ViewDataRole = "",
  [validateset('Allow', 'Deny', 'Blank')][string[]] $SaveDataRole = "",
  [validateset('Allow', 'Deny', 'Blank')][string[]] $DownloadDataRole = "",
  [validateset('Allow', 'Deny', 'Blank')][string[]] $DeleteDataRole = "",
  [validateset('Allow', 'Deny', 'Blank')][string[]] $SetDataRolePermissions = "",
...
try
 {
   if ($ScheduleID -ne ''){ $ScheduleID}else{ $ScheduleID = TS-GetScheduleDetails -name $ScheduleName}
  ...
  if ($newFrequency -ne '') 
   ...
   $response = Invoke-RestMethod -Uri ${protocol}://$server/api/$api_ver/schedules/$ScheduleID -Headers $headers -Method PUT -Body $Schedule_request
   ...
   ForEach ($detail in $response.tsresponse.schedule)
    { 
       $Schedule = [pscustomobject]@{
         ...
         Hours=$detail.frequencyDetails.intervals.interval.hours
         Minutes=$detail.frequencyDetails.intervals.interval.minutes
         DayOfMonth = $detail.frequencyDetails.intervals.interval.monthDay
       }
       $Schedule
     }
 }
...
param(
 [string[]] $ScheduleName = "",
 [string[]] $Priority ="",
 [validateset('Extract','Subscription','Flow')][string[]] $Type = "",
 [validateset('Active','Suspended')][string[]] $State = "",
...
function TS-DeleteExtractRefreshTask
{
param
 (
  [string[]] $TaskID = ""
...
# DELETE /api/api-version/sites/site-id/tasks/extractRefreshes/task-id
 try
 {
    $response = Invoke-RestMethod -Uri ${protocol}://$server/api/$api_ver/sites/$siteID/tasks/extractRefreshes/$TaskID -Headers $headers -Method DELETE
    $response.tsresponse
  }
    catch {"Unable to Delete Task: " + $_.Exception.Message}
}
...
function TS-DownloadWorkbookPDF
{
param
 (
 [string[]] $WorkBookName ="",
 ...
 [validateset('A3', 'A4', 'A5', 'B5', 'Executive', 'Folio', 'Ledger', 'Legal', 'Letter', 'Note', 'Quarto', 'Tabloid')][string[]] $PageType ="Legal",
  [string[]] $maxAge ="60"
  )
 try
  {
   ...
   $url = $protocol.trim() + "://" + $server +"/api/" + $api_ver+ "/sites/" + $siteID + "/workbooks/" + $WorkbookID + "/pdf?type="+$PageType + "&orientation=" + $Orientation + "&maxAge=" + $maxAge
   ...
   ForEach ($detail in $response.tsResponse.workbook)
     {
      ...
      ForEach ($tag in $detail.tags.tag.label){$taglist += $tag + " "}
      $Workbook = [pscustomobject]@{WorkbookName = $detail.name; WorkbookID = $detail.ID; ShowTabs=$detail.ShowTabs; ContentURL=$detail.contentURL; Size=$detail.size; CreatedAt=$detail.CreatedAt; UpdatedAt=$detail.UpdatedAt; Project=$ProjectName; Owner=$Owner; Tags=$detail.tags.tag.label; Views = $detail.Views.View.Count; ViewList =$detail.Views.View.name}
     }
      $workbook
...
function TS-UpdateWorkbook
{
 param(
  ...
  [validateset('True', 'False')][string[]] $RecentlyViewed = "",
  [validateset('True', 'False')][string[]] $EncryptExtracts = "",
  [validateset('True', 'False')][string[]] $DataAccelerationEnabled = "",
  [validateset('True', 'False')][string[]] $DataAccelerationAccelerateNow = "",
  [string[]] $WorkbookID = "",
  [string[]] $NewProjectID = ""
 ...
 try
 {
  ...
  if ($NewProjectName -ne '' -or $NewProjectID -ne '') {$body += '<project id ="'+ $NewProjectID +'" />'}
  if ($NewOwnerAccount -ne '') {$body += '<owner id ="'+ $userID +'"/>'}
 ...
 try
  {
   ...
   $url = $protocol.trim() + "://" + $server +"/api/" + $api_ver+ "/sites/" + $siteID + "/views/" + $viewID + "/" + $Suffix + $vf_filter + $join +"maxAge=1"
   $url
...
Function TS-GraphQL
{
 ...
 param(
  ...
  Invoke-WebRequest -Uri ${protocol}://$server/api/metadata/graphql -Headers $headers -Method POST -Body $Query -ContentType 'application/json' -OutFile $FileName
  


}
 Function TS-QueryTables
  {

       $response = Invoke-RestMethod -Uri ${protocol}://$server/api/$api_ver/sites/$siteID/tables -Headers $headers -Method Get
       $response.tsresponse.tables.table

  }

 Function TS-QueryColumnsInATable
  {
  param(
  [string[]]$TableID = ""
  )
       ...
       $response = Invoke-RestMethod -Uri ${protocol}://$server/api/$api_ver/sites/$siteID/tables/$TableID/columns -Headers $headers -Method Get
    ...
    Export-ModuleMember -Function TS-DeleteExtractRefreshTask
    ...
    Export-ModuleMember -Function TS-DownloadWorkbookPDF
    ...
    Export-ModuleMember -Function TS-GraphQL
    Export-ModuleMember -Function TS-QueryTables
    Export-ModuleMember -Function TS-QueryColumnsInATable
